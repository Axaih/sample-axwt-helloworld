...
goog.provide('ax.dom.ViewportSizeMonitor');
 ...
 * 
 * Use this class if you want to execute resize/reflow logic each time the user
 * resizes the browser window. This class is guaranteed to only dispatch
 ...
 * 
 * @param {Window=}
 *            opt_window The window to monitor; defaults to the window in which
 *            this code is executing.
 * @constructor
 * @extends {goog.events.EventTarget}
 */
ax.dom.ViewportSizeMonitor = function(opt_window) {
	...
	if (this.isPollingRequired_()) {
		this.windowSizePollInterval_ = window.setInterval(goog.bind(
				this.checkForSizeChange_, this),
				ax.dom.ViewportSizeMonitor.WINDOW_SIZE_POLL_RATE);
	}
};
goog.inherits(ax.dom.ViewportSizeMonitor, goog.events.EventTarget);

/**
 * Returns a viewport size monitor for the given window. A new one is created if
 * it doesn't exist already. This prevents the unnecessary creation of multiple
 * spooling monitors for a window.
 * 
 * @param {Window=}
 *            opt_window The window to monitor; defaults to the window in which
 *            this code is executing.
 ...
 */
ax.dom.ViewportSizeMonitor.getInstanceForWindow = function(opt_window) {
	var currentWindow = opt_window || window;
	var uid = goog.getUid(currentWindow);

	return goog.dom.ViewportSizeMonitor.windowInstanceMap_[uid] = goog.dom.ViewportSizeMonitor.windowInstanceMap_[uid]
 ...
 * 
 * @param {Window=}
 *            opt_window The window whose monitor should be removed; defaults to
 *            the window in which this code is executing.
 */
ax.dom.ViewportSizeMonitor.removeInstanceForWindow = function(opt_window) {
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.windowInstanceMap_ = {};

/**
 * Rate in milliseconds at which to poll the window size on browsers that need
 * polling.
 * 
 * @type {number}
 */
ax.dom.ViewportSizeMonitor.WINDOW_SIZE_POLL_RATE = 100;
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.listenerKey_ = null;
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.window_ = null;
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.size_ = null;
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.windowSizePollInterval_ = null;
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.isPollingRequired_ = function() {
	/*
 ...
 */
ax.dom.ViewportSizeMonitor.prototype.getSize = function() {
	// Return a clone instead of the original to preserve encapsulation.
	return this.size_ ? this.size_.clone() : null;
};

/** @override */
ax.dom.ViewportSizeMonitor.prototype.disposeInternal = function() {
	ax.dom.ViewportSizeMonitor.superClass_.disposeInternal.call(this);
...
/**
 * Handles window resize events by measuring the dimensions of the viewport and
 * dispatching a {@link goog.events.EventType.RESIZE} event if the current
 * dimensions are different from the previous ones.
 * 
 * @param {goog.events.Event}
 *            event The window resize event to handle.
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.handleResize_ = function(event) {
 ...
 * @private
 */
ax.dom.ViewportSizeMonitor.prototype.checkForSizeChange_ = function() {
